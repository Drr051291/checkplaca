...
export const HeroSection = () => {
  ...
  const [planType, setPlanType] = useState<'basico' | 'premium'>('basico');
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Se for premium, envia planType, senão não envia (consulta grátis)
      const requestBody: any = { plate: cleanedPlate };
      if (planType === 'premium') {
              ...
              <CardContent className="p-4 sm:p-6 md:p-8">
                <form onSubmit={handleSearch} className="space-y-4 sm:space-y-6">
                      ...
                      <RadioGroup value={planType} onValueChange={(value: any) => setPlanType(value)} className="flex flex-col gap-2">
                        ...
                        <div className={`flex items-center space-x-2 p-3 rounded-lg border-2 cursor-pointer transition-smooth ${planType === 'premium' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`} onClick={() => setPlanType('premium')}>
                          ...
                          <Label htmlFor="premium" className="flex-1 cursor-pointer">
                            ...
                            <div className="text-xs text-muted-foreground">Relatório completo: leilão, sinistros, débitos e histórico</div>
                          </Label>
                        </div>
                      </RadioGroup>
                    </div>
                    ...
                    >
                      {isSearching ? (
                        ...
                        <>
                          ...
                          <span className="hidden sm:inline">{planType === 'premium' ? 'Gerar Relatório Premium' : 'Consultar Veículo Grátis'}</span>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground text-center">
                    {planType === 'premium' ? '✓ Teste sem pagamento • ✓ Relatório completo' : '✓ Consulta gratuita • ✓ Resultado instantâneo'}
                  </p>
                </form>
              </CardContent>
            </Card>
